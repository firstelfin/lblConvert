...
from lblConvert.tools import read_yolo, save_yolo_label
...
class YoloLabelExclude(object):
    ...
    def save_classes(self, save_dir: Optional[PathStr] = None):
        """保存新的classes.txt文件

        :param str|Path save_dir: 保存地址, defaults to None
        """
        if save_dir is None:
    ...
    def save_yaml(self, save_dir: Optional[PathStr] = None):
        """保存新标签文件
        
        :param str|Path save_dir: 新标签保存地址
        """
    ...
    def label_exclude(
            self, lbl_path: PathStr, img_save_dir: Optional[PathStr] = None, 
            lbl_save_dir: Optional[PathStr] = None, img_path: Optional[PathStr] = None, 
            cp_img: bool = False
        ):
        ...
        # 保存新标签
        if lbl_save_dir is None:
            raise ValueError("lbl_save_dir must be set.")
        Path(lbl_save_dir).mkdir(parents=True, exist_ok=True)
        save_yolo_label(Path(lbl_save_dir) / Path(lbl_path).name, new_lbl_list)
        if cp_img:
            if img_save_dir is None or img_path is None:
                raise ValueError("img_save_dir must be set when cp_img is True.")
            Path(img_save_dir).mkdir(parents=True, exist_ok=True)
            shutil.copy(img_path, Path(img_save_dir) / Path(img_path).name)

    def exclude_classes_by_subset(
            self, img_save_dir: Optional[PathStr], lbl_save_dir: PathStr, 
            img_dir: PathStr, lbl_dir: PathStr, cp_img: bool = False
        ):
        """根据子集的图片和标签文件, 剔除不在include_classes中的类别, 并重置类别序号

        :param str|Path img_save_dir: 图片保存的地址
        :param str|Path lbl_save_dir: 标签保存的地址
        :param str|Path img_dir: 原子集图片的地址
        :param str|Path lbl_dir: 原子集标签的地址
        :param bool cp_img: 是否复制图片, defaults to False
        """
        results = []
        with ThreadPoolExecutor() as executor:
            for img_file in Path(img_dir).iterdir():
                lbl_file = Path(lbl_dir) / f'{img_file.stem}.txt'
    ...
    def __call__(self, dst_dir: Optional[PathStr] = None, cp_img: bool = False):
        """标签转换接口
